...
class MELNOUVEAU_Props_panel_walls(bpy.types.PropertyGroup):
    pass


def register_props():
    props_cls = MELNOUVEAU_Props_panel_walls
    for gen_id, gen_name in [('whiplash_wall', 'Whiplash Wall'), ('curved_facade', 'Curved Facade')]:
...
MELNOUVEAU_Props_panel_walls._generators = [('whiplash_wall', 'Whiplash Wall'), ('curved_facade', 'Curved Facade')]


class MELNOUVEAU_PT_panel_walls(bpy.types.Panel):
    bl_label = "Walls.Py"
    bl_idname = "MELNOUVEAU_PT_panel_walls"
    ...
    def draw(self, context):
        ...
        for gen_id, gen_name in MELNOUVEAU_Props_panel_walls._generators:
            gen_cls = get_generator(gen_id)
            if not gen_cls:
                continue
            box = layout.box()
...
_classes = [MELNOUVEAU_Props_panel_walls, MELNOUVEAU_PT_panel_walls]
