...
class MELNOUVEAU_Props_panel_columns(bpy.types.PropertyGroup):
    pass


def register_props():
    props_cls = MELNOUVEAU_Props_panel_columns
    for gen_id, gen_name in [('stem_column', 'Stem Column'), ('tree_column', 'Tree Column')]:
...
MELNOUVEAU_Props_panel_columns._generators = [('stem_column', 'Stem Column'), ('tree_column', 'Tree Column')]


class MELNOUVEAU_PT_panel_columns(bpy.types.Panel):
    bl_label = "Columns.Py"
    bl_idname = "MELNOUVEAU_PT_panel_columns"
    ...
    def draw(self, context):
        ...
        for gen_id, gen_name in MELNOUVEAU_Props_panel_columns._generators:
            gen_cls = get_generator(gen_id)
            if not gen_cls:
                continue
            box = layout.box()
...
_classes = [MELNOUVEAU_Props_panel_columns, MELNOUVEAU_PT_panel_columns]
