...
function Get-Expr($path) {
    for ($attempt = 1; $attempt -le 8; $attempt++) {
        ...
        try {
            ...
            if ($parsed.expressions -and $parsed.expressions.Count -gt 0) {
                Write-Host "    get_all_expressions OK on attempt $attempt"
        ...
        if ($er) {
            try {
                ...
                if ($exp.nodes -and $exp.nodes.Count -gt 0) {
                    Write-Host "    export fallback on attempt $attempt"
                    $exprs = @()
                    $classCount = @{}
                    foreach ($n in $exp.nodes) {
                        $cn = $n.class
                        if (-not $classCount.ContainsKey($cn)) { $classCount[$cn] = 0 }
                        $idx = $classCount[$cn]; $classCount[$cn]++
                        $fn = if ($n.name -and $n.name.Length -gt 0) { $n.name } else { "MaterialExpression$cn`_$idx" }
                        $exprs += @{ name = $fn; class = "MaterialExpression$cn" }
                    }
                    foreach ($n in $exp.custom_hlsl_nodes) {
                        $fn = if ($n.name -and $n.name.Length -gt 0) { $n.name } else { "MaterialExpressionCustom_0" }
                        $exprs += @{ name = $fn; class = "MaterialExpressionCustom" }
                    }
                    return $exprs
                }
            } catch {}
...
$fullPath = "/Game/_PROJECT/04_Materials/SDF/M_SDF_FloatingNotes"
Write-Host "=== FULL REBUILD M_SDF_FloatingNotes ===" -ForegroundColor Cyan

# Force delete
Call-Mono "delete_asset" @{ asset_path = $fullPath } | Out-Null
Write-Host "  Deleted, waiting..."
Start-Sleep -Seconds 5
...
$nodes = @(
    ...
    @{ id = "NA"; class = "VectorParameter"; pos = @(-1200, -80); props = @{ ParameterName = "NoteColorA" } },
    @{ id = "NB"; class = "VectorParameter"; pos = @(-1200, -20); props = @{ ParameterName = "NoteColorB" } },
    @{ id = "AC"; class = "VectorParameter"; pos = @(-1200, 40); props = @{ ParameterName = "AccentColor" } },
    @{ id = "GI"; class = "ScalarParameter"; pos = @(-1200, 100); props = @{ ParameterName = "GlowIntensity" } }
)
...
$inputObjs = @(@{name="UV"},@{name="Time"},@{name="NoteColorA"},@{name="NoteColorB"},@{name="AccentColor"},@{name="GlowIntensity"})
$r = Call-Mono "create_custom_hlsl_node" @{ asset_path = $fullPath; code = $code; description = "FloatingNotes"; output_type = "CMOT_Float3"; inputs = $inputObjs }
if (-not $r) { Write-Host "  FATAL: null" -ForegroundColor Red; exit 1 }
$cj = $r | ConvertFrom-Json
Write-Host "  Custom=$($cj.expression_name) inputs=$($cj.input_count)"
Start-Sleep -Seconds 12

$exprs = Get-Expr $fullPath
if (-not $exprs) { Write-Host "  FATAL: no expressions" -ForegroundColor Red; exit 1 }
...
$inputNames = @("UV","Time","NoteColorA","NoteColorB","AccentColor","GlowIntensity")
...
if ($r) {
    try {
        ...
        if ($s.is_compiled) {
            Write-Host "  COMPILED: $($s.num_pixel_shader_instructions) PS" -ForegroundColor Green
