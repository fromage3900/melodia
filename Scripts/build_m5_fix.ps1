    ...
    for ($attempt = 1; $attempt -le 8; $attempt++) {
        ...
        try {
            ...
            if ($parsed.expressions -and $parsed.expressions.Count -gt 0) {
                Write-Host "    get_all_expressions succeeded on attempt $attempt"
        ...
        if ($er) {
            try {
                ...
                if ($exp.nodes -and $exp.nodes.Count -gt 0) {
                    Write-Host "    Using export fallback on attempt $attempt"
                    $exprs = @()
                    $classCount = @{}
                    foreach ($n in $exp.nodes) {
                        $className = $n.class
                        if (-not $classCount.ContainsKey($className)) { $classCount[$className] = 0 }
                        $idx = $classCount[$className]
                        $classCount[$className]++
                        $fullName = if ($n.name -and $n.name.Length -gt 0) { $n.name } else { "MaterialExpression$className`_$idx" }
                        $exprs += @{ name = $fullName; class = "MaterialExpression$className" }
                    }
                    foreach ($n in $exp.custom_hlsl_nodes) {
                        $fullName = if ($n.name -and $n.name.Length -gt 0) { $n.name } else { "MaterialExpressionCustom_0" }
                        $exprs += @{ name = $fullName; class = "MaterialExpressionCustom" }
                    }
                    return $exprs
                }
            } catch {}
