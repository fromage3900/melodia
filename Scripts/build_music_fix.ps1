...
function Build-SDF($name, $paramDefs, $hlslCode, $inputNames) {
    ...
    Write-Host "  Deleted old (if exists)..."
    ...
    Write-Host "  Graph built, waiting for server settle..."
    ...
    Write-Host "  Waiting before get_all_expressions..."
    Start-Sleep -Seconds 8
    $exprs = Get-Expressions-Retry $fullPath
    if (-not $exprs) { Write-Host "  FATAL: Could not get expressions after 6 retries" -ForegroundColor Red; return }
...
for(int step=0;step<80;step++){
  ...
  for(int ni=0;ni<10;ni++){
    float frac01=float(ni)/9.0;
    float nx=lerp(-0.9,0.95,frac01);
...
# ============================================================
# M3: M_SDF_FloatingNotes (FIXED: fract->frac)
# ============================================================
$floatNotesCode = @'
float2 uvc=(UV-0.5)*2.0;
...
Write-Host "`n=== BATCH 1 (M1-M3) COMPLETE ===" -ForegroundColor Yellow
