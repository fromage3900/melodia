...
Write-Host "=== REBUILD M3 (step->si fix) ===" -ForegroundColor Cyan
...
# FIXED: step->si, int noteKind->float noteKindF
$code = @'
...
for(int si=0;si<85;si++){
  ...
  for(int ni=0;ni<8;ni++){
    ...
    note=min(note,stem);
    float noteKindF=floor(fmod(fi,3.0));
    if(noteKindF>0.5&&noteKindF<1.5){
