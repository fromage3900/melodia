...
$ct = "application/json"
$assetPath = "/Game/_PROJECT/04_Materials/SDF/M_SDF_Mandelbulb_Master"
...
$createBody = @{
    ...
    params = @{
        ...
        arguments = @{
            ...
            inputs = @("UV", "Time", "Power", "MaxIter", "SliceOff", "RotSpd", "ScaleParam", "AOStr", "BaseCol", "GlowCol", "InkCol")
        }
    }
} | ConvertTo-Json -Depth 20 -Compress

Write-Host "Creating custom HLSL node..."
$r = Invoke-RestMethod -Uri $baseUrl -Method Post -Body $createBody -ContentType $ct -TimeoutSec 60
$text = $r.result.content | Where-Object { $_.type -eq "text" } | Select-Object -First 1
Write-Host "Result: $($text.text)" -ForegroundColor Cyan
