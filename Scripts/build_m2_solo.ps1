...
function Wait-Server {
    for ($i = 0; $i -lt 40; $i++) {
        try {
            ...
            $null = Invoke-RestMethod -Uri $baseUrl -Method Post -Body $body -ContentType $ct -TimeoutSec 10
            return $true
...
function Build-SDF($name, $paramDefs, $hlslCode, $inputNames) {
    ...
    Write-Host "  Waiting 12s before getting expressions..."
    Start-Sleep -Seconds 12
    $exprs = Get-Expr $fullPath
    if (-not $exprs) { Write-Host "  FATAL: no expressions after 8 retries" -ForegroundColor Red; return $false }
    ...
    Write-Host "  Got $($paramExprs.Count) param nodes, wiring $($inputNames.Count) pins..."
...
return saturate(col);
'@
$ok = Build-SDF "M_SDF_TrebleClef_Ornament" @(
...
Call-Mono "save_material" @{ asset_path = "$sdfPath/M_SDF_TrebleClef_Ornament" } | Out-Null
Write-Host "M2 result: $(if($ok){'OK'}else{'FAIL'})" -ForegroundColor $(if($ok){'Green'}else{'Red'})
